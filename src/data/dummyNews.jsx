<<<<<<< HEAD
=======
// src/data/dummyNews.js

>>>>>>> 6b71656 (update project)
export const allNewsData = [
  {
    id: "1",
    date: "08 OKT 2025",
    category: "ENVIRONMENT",
    title: "Heningnya Udara Hutan Kalimantan Timur Menarik Minat Investor BUMS",
    description: "Analisis mendalam mengenai bagaimana institusi pendidikan dan kebudayaan beradaptasi dengan kecepatan algoritma tanpa kehilangan esensi tradisi.",
    summary: "Hutan Kalimantan Timur kembali menarik perhatian, bukan hanya karena keanekaragaman hayatinya, tetapi sebagai aset ekonomi hijau global...",
    excerpt: "Mengulas peran penting hutan lintang tinggi dalam siklus karbon global seiring meningkatnya suhu bumi.",
    author: "Faiz Alif",
    handle: "@FAIZALIF",
    image: "https://images.unsplash.com/photo-1441974231531-c6227db76b6e",
    heroImage: "https://picsum.photos/seed/forest/1200/600",
    authorImage: "https://picsum.photos/seed/user1/40/40",
    readTime: "5 min read",
<<<<<<< HEAD
=======
    reads: 34200, // Data untuk SidebarTrending
>>>>>>> 6b71656 (update project)
    reactions: { clap: 142, light: 86, idea: 54 }
  },
  {
    id: "2",
    date: "10 MEI 2026",
    category: "POLITICS",
    title: "Presiden Prabowo Sibuk Rayakan Ulang Tahun Buna Teddy",
    description: "Evaluasi menyeluruh terhadap sistem administrasi negara dalam menghadapi dinamika politik global.",
    summary: "Kemeriahan perayaan ulang tahun ke-29 Buna Teddy menjadi sorotan publik di tengah agenda kenegaraan yang padat...",
    excerpt: "Bagaimana momen personal pemimpin negara menjadi wajah baru komunikasi politik di era digital.",
    author: "Tirta",
    handle: "@TIRTIR",
    image: "https://picsum.photos/seed/teddy/1200/600",
    heroImage: "https://picsum.photos/seed/teddy/1200/600",
    authorImage: "https://picsum.photos/seed/user2/40/40",
    readTime: "3 min read",
<<<<<<< HEAD
=======
    reads: 28900,
>>>>>>> 6b71656 (update project)
    reactions: { clap: 50, light: 20, idea: 15 }
  },
  {
    id: "3",
    date: "08 OKT 2025",
    category: "ECONOMICS",
    title: "Transformasi Ekonomi Hijau: Bagaimana Indonesia Memimpin Arus Asia Tenggara",
    description: "Langkah berani dalam dekarbonisasi industri menempatkan nusantara di garis depan revolusi industri berkelanjutan.",
    excerpt: "Model bisnis baru yang berfokus pada daur ulang sampah plastik menjadi produk bernilai tinggi.",
    author: "Linda Kusuma",
    image: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab",
<<<<<<< HEAD
    readTime: "3 min read"
=======
    readTime: "3 min read",
    reads: 15400
>>>>>>> 6b71656 (update project)
  },
  {
    id: "4",
    date: "09 OKT 2025",
    category: "TECHNOLOGY",
    title: "Etika Kecerdasan Buatan dalam Kehidupan Sehari-hari",
    description: "Bagaimana AI mulai mengambil peran dalam pengambilan keputusan moral manusia.",
    excerpt: "Melihat sejauh mana algoritma bisa dipercaya untuk menentukan pilihan etis kita.",
    author: "Sari Wijaya",
    image: "https://images.unsplash.com/photo-1550745165-9bc0b252726f",
<<<<<<< HEAD
    readTime: "2 min read"
=======
    readTime: "2 min read",
    reads: 19500
>>>>>>> 6b71656 (update project)
  },
  {
    id: "5",
    date: "10 OKT 2025",
    category: "SOCIAL",
    title: "Seni Hidup Lambat di Tengah Kecepatan Metropolitan",
    description: "Di tengah hiruk-pikuk kota metropolitan, hidup sering terasa seperti perlombaan tanpa garis akhir.",
    excerpt: "Menemukan ketenangan dalam rutinitas urban yang melelahkan melalui filosofi slow living.",
    author: "Rian",
    image: "https://picsum.photos/seed/nature/400/300",
<<<<<<< HEAD
    readTime: "4 min read"
=======
    readTime: "4 min read",
    reads: 12000
>>>>>>> 6b71656 (update project)
  },
  {
    id: "6",
    date: "11 OKT 2025",
    category: "DESIGN",
    title: "Etika Materialitas: Masa Depan Komponen Mewah",
    description: "Menelusuri asal-usul komponen mewah dan tuntutan konsumen terhadap transparansi yang lebih radikal.",
    excerpt: "Kenapa brand global mulai beralih ke material daur ulang untuk produk high-end mereka.",
    author: "Andi Pratama",
    image: "https://images.unsplash.com/photo-1523275335684-37898b6baf30",
<<<<<<< HEAD
    readTime: "2 min read"
=======
    readTime: "2 min read",
    reads: 8900
>>>>>>> 6b71656 (update project)
  },
  {
    id: "7",
    date: "12 OKT 2025",
    category: "EDUCATION",
    title: "Terobosan Genetik dalam Mitigasi Penyakit Tropis",
    description: "Inovasi medis terbaru yang memanfaatkan teknologi CRISPR.",
    excerpt: "Harapan baru bagi pemberantasan penyakit endemik di wilayah tropis melalui rekayasa genetika.",
    author: "Dr. Hani",
    image: "https://picsum.photos/seed/science/400/300",
<<<<<<< HEAD
    readTime: "5 min read"
  }
];

// Data Sidebar & Komentar (Tetap dipisah karena tipenya beda)
=======
    readTime: "5 min read",
    reads: 21000
  }
];

// --- DATA UNTUK SIDEBAR & KOMENTAR ---

>>>>>>> 6b71656 (update project)
export const dummyComments = [
  { id: 1, author: "Fahri Sani", time: "2 hours ago", text: "Harusnya Lebih Dilestarikan dong bukan dijual gini gimana sih pemerintah" },
  { id: 2, author: "Vennanda", time: "2 hours ago", text: "Gak setuju am komen atas gw, kita butuh 1000rb lapangan pekerjaan" }
];

export const popularData = [
  { id: "2", category: "POLITICS", title: "Presiden Prabowo Sibuk Rayakan Ulang Tahun Buna Teddy", readTime: "3 MIN READ" },
  { id: "4", category: "TECHNOLOGY", title: "Neural Architecture: The Next Phase of AI", readTime: "2 MIN READ" },
<<<<<<< HEAD
  { id: "5", category: "CULTURE", title: "Rahasia Tradisi Malam Jum'at Kliwon di Kota Batang", readTime: "3 MIN READ" }
];

export const moreData = [
  { id: "8", title: "Regenerasi Agrikultur: Apakah Bisa?", date: "12 JANUARI 2026", img: "https://picsum.photos/seed/agri1/60/60" },
  { id: "9", title: "Tanam Ubi Di Pasir Untung Jutaan Rupiah", date: "10 JANUARI 2026", img: "https://picsum.photos/seed/ubi2/60/60" }
];
=======
  { id: "3", category: "ECONOMICS", title: "Transformasi Ekonomi Hijau di Indonesia", readTime: "3 MIN READ" }
];

export const moreData = [
  { id: "1", title: "Regenerasi Agrikultur: Apakah Bisa?", date: "12 JANUARI 2026", img: "https://picsum.photos/seed/agri1/60/60" },
  { id: "5", title: "Seni Hidup Lambat di Kota", date: "10 JANUARI 2026", img: "https://picsum.photos/seed/nature/60/60" }
];

// --- DATA KHUSUS TRENDING SIDEBAR (DENGAN READS) ---
export const trendingSidebarData = allNewsData
  .slice(0, 3) // Ambil 3 teratas
  .map(item => ({
    id: item.id,
    title: item.title,
    reads: item.reads || 0
  }));
>>>>>>> 6b71656 (update project)
